...
try {
  ...
  if (-not $NoShortcuts) {
    ...
    $restoreBaseArgument = if ($SkipBaseTheme) { '' } else { ' -RestoreBaseTheme' }

    foreach ($folder in @($desktop, $startMenu)) {
      $shortcut = $shell.CreateShortcut((Join-Path $folder 'Codex 2010 Skin.lnk'))
      $shortcut.TargetPath = $powershell
      $shortcut.Arguments = "-NoProfile -ExecutionPolicy Bypass -File `"$startScript`"$portArgument -PromptRestart"
      $shortcut.WorkingDirectory = $SkillRoot
      $shortcut.Description = 'Launch the official Codex app with the QQ 2010 inspired skin'
      $shortcut.Save()
    }

    $restore = $shell.CreateShortcut((Join-Path $desktop 'Codex 2010 Skin - Restore.lnk'))
    $restore.TargetPath = $powershell
    $restore.Arguments = "-NoProfile -ExecutionPolicy Bypass -File `"$restoreScript`"$portArgument$restoreBaseArgument -PromptRestart"
    $restore.WorkingDirectory = $SkillRoot
    $restore.Description = 'Restore the official Codex appearance and close the local skin session'
    $restore.Save()
  }

  if ($NoShortcuts) {
    Write-Host 'Codex 2010 Skin prepared. Run start-dream-skin.ps1 to launch it.'
  } else {
    Write-Host 'Codex 2010 Skin installed. The launch shortcut asks before restarting an open Codex window.'
  }
} finally {
