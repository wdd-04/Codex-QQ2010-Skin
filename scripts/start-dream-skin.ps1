...
try {
  ...
  if (-not $debugReady -and $codexProcesses.Count -gt 0) {
    ...
    if (-not $restartAuthorized -and $PromptRestart) {
      $restartAuthorized = Confirm-DreamSkinRestart -Message 'Codex must restart once to enable the 2010 skin. Unsaved input may be lost. Restart now?'
  ...
  try {
    ...
    # Native stderr is useful verification output. Under Windows PowerShell 5.1,
    # ErrorActionPreference=Stop can otherwise turn it into a terminating error
    # before we can capture the process exit code and write verify.log.
