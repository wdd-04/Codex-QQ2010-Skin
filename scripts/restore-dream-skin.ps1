...
try {
  ...
  if ($shouldCloseCodex -and $PromptRestart) {
    $restartMessage = if ($NoRelaunch) {
      'Restore will close Codex, remove the 2010 skin, and end its local skin session. Continue?'
    } else {
      'Restore will close Codex, remove the 2010 skin and its local session, then reopen the official app. Continue?'
    }
  ...
  try {
    ...
    if ($Uninstall) {
      ...
      @(
        (Join-Path $desktop 'Codex 2010 Skin.lnk'),
        (Join-Path $desktop 'Codex 2010 Skin - Restore.lnk'),
        (Join-Path $startMenu 'Codex 2010 Skin.lnk')
  ...
  Write-Host 'Codex 2010 Skin restore completed; any saved local skin session was closed.'
} finally {
